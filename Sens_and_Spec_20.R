...
Threshold_3y <- c(0.56, 0.70, 0.77, 0.89, 1.03, 2.92)
...
for (i in 1:length(Threshold_3y)) {
  tp <- nrow(risk_20_at_3y_order[risk_20_at_3y_order$Status=="CBC" & risk_20_at_3y_order$Risk >= Threshold_3y[i],])
  fn <- nrow(risk_20_at_3y_order[risk_20_at_3y_order$Status=="CBC" & risk_20_at_3y_order$Risk < Threshold_3y[i],])
  p <- tp+fn
  sens <- tp/p
...
Specificity <- c()
for (i in 1:length(Threshold_3y)) {
  tn <- nrow(risk_20_at_3y_order[risk_20_at_3y_order$Status=="No CBC" & risk_20_at_3y_order$Risk < Threshold_3y[i],])
  fp <- nrow(risk_20_at_3y_order[risk_20_at_3y_order$Status=="No CBC" & risk_20_at_3y_order$Risk >= Threshold_3y[i],])
  n <- tn + fp
  spec <- tn/n
  Specificity <- append(Specificity, spec, i-1)
}
risk_20_at_3y_thresh <- rbind(Threshold_3y, Sensitivity, Specificity)
write.csv(risk_20_at_3y_thresh, file = "risk_20_at_3y_thresh.csv")
...
Threshold_5y <- c(0.94, 1.13, 1.25, 1.42, 1.71, 4.74)
...
for (i in 1:length(Threshold_5y)) {
  tp <- nrow(risk_20_at_5y_order[risk_20_at_5y_order$Status=="CBC" & risk_20_at_5y_order$Risk >= Threshold_5y[i],])
  fn <- nrow(risk_20_at_5y_order[risk_20_at_5y_order$Status=="CBC" & risk_20_at_5y_order$Risk < Threshold_5y[i],])
  p <- tp+fn
  sens <- tp/p
  Sensitivity <- append(Sensitivity, sens, i-1)
}
...
Specificity <- c()
for (i in 1:length(Threshold_5y)) {
  tn <- nrow(risk_20_at_5y_order[risk_20_at_5y_order$Status=="No CBC" & risk_20_at_5y_order$Risk < Threshold_5y[i],])
  fp <- nrow(risk_20_at_5y_order[risk_20_at_5y_order$Status=="No CBC" & risk_20_at_5y_order$Risk >= Threshold_5y[i],])
  n <- tn + fp
  spec <- tn/n
  Specificity <- append(Specificity, spec, i-1)
}
risk_20_at_5y_thresh <- rbind(Threshold_5y, Sensitivity, Specificity)
write.csv(risk_20_at_5y_thresh, file = "risk_20_at_5y_thresh.csv")
